...
void calc(int i, float samplingColCount, vec3 gPosition, vec3 gNormal, inout vec3 indirectLC, inout float shelter)
{
    float y = floor(float(i) / samplingColCount);
    float x = float(i) - y * samplingColCount;
    
    vec4 rand = texture(tRandNum, vec2(x / (samplingColCount - 1.0), y / (samplingColCount - 1.0)));
    vec3 vplPosition = gPosition + rand.xyz * 255.0 - 127.0;
    vec4 posInValMap = projMatrix * viewMatrix * vec4(vplPosition, 1.0);
    vec2 tex = (0.5 * posInValMap.xy + 0.5) / posInValMap.w;
...
void main()
{
	...
	for (int i = 0; i< samplingTotalNum; ++i)
	{
        ...
        calc(i, float(samplingColCount), gPosition.xyz, gNormal.xyz, indirectLC, shelter);
    }
